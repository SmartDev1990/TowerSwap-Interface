...
const masterChefFarmCalls = async (farm: SerializedFarm) => {
  ...
  const multiCallChainId = farmFetcher.isTestnet(quoteToken.chainId) ? ChainId.SHARDEUMV : ChainId.ZETA
...
export const fetchMasterChefData = async (farms: SerializedFarmConfig[], chainId: number): Promise<any[]> => {
  ...
  const multiCallChainId = farmFetcher.isTestnet(chainId) ? ChainId.SHARDEUMV : ChainId.ZETA
